...
DB* db_open_ex(const char* basedir, uint64_t cache_size)
{
    ...
    pthread_mutex_init(&self->my_mutex,NULL); //Arxikopoisi tou mutex
    
    pthread_cond_init(&self->turn, NULL); //Arxikopoisi tis metablitis sinithikis.
...
{
    int add_x; //metavliti pou xreiazomaste gia tin sosti leitourgia ths db_add
    ...
    self->writing = self->writing + 1; //Efoson mporw na synexisw ayxanw autous poy grafun kata 1
    ...
    pthread_mutex_lock(&self->my_mutex); //Afou oloklirwthei h leitourgia kleidwnw to mutex
    
    self->writing = self->writing - 1; //Afairw oti auxisa parapanw
    self->writers = self->writers - 1; //Afairw oti auxisa parapanw afou i diadikasia oloklirwthike
    
    pthread_cond_broadcast(&self->turn); //Shmatodvtv ta upoloipa nimata oti h leitoyrgia oloklirwthike
